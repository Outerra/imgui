...
namespace ImGui
{
    ...
    IMGUI_API bool          ClearWindowFocusedFlag();                                                   // returns true if any window was focused since the last call to this function or to NewFrame
    ...
    IMGUI_API bool          CollapsingHeader(const char* label, const char* label_end, ImGuiTreeNodeFlags flags = 0);  // if returning 'true' the header is open. doesn't indent nor push on ID stack. user doesn't have to call TreePop().
    ...
    IMGUI_API bool          IsKeyPressed(int user_key_index, bool repeat = false);              // was key pressed (went from !Down to Down)? if repeat=true, uses io.KeyRepeatDelay / KeyRepeatRate
