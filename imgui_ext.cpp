...
void StyleColorsOuterra(ImGuiStyle* dst)
{
    ...
    colors[ImGuiCol_TitleBgActive] = ImVec4(0.85f, 0.35f, 0.00f, 1.00f);
    ...
    colors[ImGuiCol_ButtonActive] = ImVec4(0.30f, 0.60f, 0.70f, 1.00f);
    ...
    colors[ImGuiCol_Separator] = ImVec4(1.00f, 0.50f, 0.00f, 1.00f);
    colors[ImGuiCol_SeparatorHovered] = ImVec4(0.75f, 0.37f, 0.00f, 1.00f);
    colors[ImGuiCol_SeparatorActive] = ImVec4(0.50f, 0.25f, 0.00f, 1.00f);
...
void TextClipped(ImStrv text, float max_width, ImGuiExTextClippedFlags flags)
{
    ...
    {
        ImGui::RenderTextEllipsis(ImGui::GetWindowDrawList(), textRect.Min, textRect.Max, textRect.Max.x, text, &textSize);
...
bool InputFloatForced(ImStrv label, bool& forced, float* v, float step, float step_fast, const char* format, ImGuiInputTextFlags flags) {
    return core_gui<&ImGui::InputFloat>(forced, label, v, step, step_fast, format, flags);
}


bool InputIntForced(ImStrv label, bool& forced, int* v, int step, int step_fast, ImGuiInputTextFlags flags) {
    return core_gui<&ImGui::InputInt>(forced, label, v, step, step_fast, flags);
}

bool InputInt64Forced(ImStrv label, bool& forced, int64* v, int64 step, int64 step_fast, ImGuiInputTextFlags flags) {
    return core_gui<&ImGui::InputScalar>(forced, label, ImGuiDataType_S64, v, &step, &step_fast, "%d", flags);
}
...
bool LabelCheck(ImStrv label, bool* checkbox)
{
    ...
    if (ImGui::ItemAdd(textRect, window->GetID(label)))
    {
        ...
        ImGui::RenderTextEllipsis(ImGui::GetWindowDrawList(), textRect.Min, textRect.Max, textRect.Max.x, label, &textSize);
...
bool SliderU8(ImStrv label, uint8* v, const char* format, ImGuiSliderFlags flags)
{
    ImGuiEx::Label(label);
    ImGui::PushID(label);
    uint8 min = 0, max = UINT8_MAX;
    bool result = ImGui::SliderScalar("##SliderU8", ImGuiDataType_U8, v, &min, &max, format, flags);
    ImGui::PopID();
    return result;
}
...
bool SliderU8Vec2(ImStrv label, uint8 v[2], uint8 v_min, uint8 v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    ImGui::PushID(label);
    uint8 min = 0, max = UINT8_MAX;
    bool result = ImGui::SliderScalarN("##SliderU8Vec2", ImGuiDataType_U8, v, 2, &min, &max, format, flags);
    ImGui::PopID();
    return result;
}
bool SliderU8Vec3(ImStrv label, uint8 v[3], uint8 v_min, uint8 v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    ImGui::PushID(label);
    uint8 min = 0, max = UINT8_MAX;
    bool result = ImGui::SliderScalarN("##SliderU8Vec3", ImGuiDataType_U8, v, 3, &min, &max, format, flags);
    ImGui::PopID();
    return result;
}
bool SliderU8Vec4(ImStrv label, uint8 v[4], uint8 v_min, uint8 v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    ImGui::PushID(label);
    uint8 min = 0, max = UINT8_MAX;
    bool result = ImGui::SliderScalarN("##SliderU8Vec4", ImGuiDataType_U8, v, 4, &min, &max, format, flags);
    ImGui::PopID();
    return result;
}

bool SliderU8Norm(ImStrv label, uint8* v, const char* format, ImGuiSliderFlags flags)
{
    ImGuiEx::Label(label);
    ImGui::PushID(label);
    float fv = *v * (1.0f / 255);
    bool result = ImGui::SliderFloat("##SliderU8Norm", &fv, 0.0f, 1.0f, format, flags);
    if (result)
        *v = uint8(round(fv * 255));
    ImGui::PopID();
    return result;
}

bool SliderU8Vec2Norm(ImStrv label, uint8 v[2], const char* format, ImGuiSliderFlags flags)
{
    ImGuiEx::Label(label);
    ImGui::PushID(label);
    float fv[2] = {v[0] * (1.0f / 255), v[1] * (1.0f / 255)};
    bool result = ImGui::SliderFloat2("##SliderU8Vec2Norm", fv, 0.0f, 1.0f, format, flags);
    if (result) {
        v[0] = uint8(round(fv[0] * 255));
        v[1] = uint8(round(fv[1] * 255));
    }
    ImGui::PopID();
    return result;
}
bool SliderU8Vec3Norm(ImStrv label, uint8 v[3], const char* format, ImGuiSliderFlags flags)
{
    ImGuiEx::Label(label);
    ImGui::PushID(label);
    float fv[3] = {v[0] * (1.0f / 255), v[1] * (1.0f / 255), v[2] * (1.0f / 255)};
    bool result = ImGui::SliderFloat3("##SliderU8Vec3Norm", fv, 0.0f, 1.0f, format, flags);
    if (result) {
        v[0] = uint8(round(fv[0] * 255));
        v[1] = uint8(round(fv[1] * 255));
        v[2] = uint8(round(fv[2] * 255));
    }
    ImGui::PopID();
    return result;
}
bool SliderU8Vec4Norm(ImStrv label, uint8 v[4], uint8 v_min, uint8 v_max, const char* format, ImGuiSliderFlags flags)
{
    ImGuiEx::Label(label);
    ImGui::PushID(label);
    float fv[4] = {v[0] * (1.0f / 255), v[1] * (1.0f / 255), v[2] * (1.0f / 255), v[3] * (1.0f / 255)};
    bool result = ImGui::SliderFloat4("##SliderU8Vec4Norm", fv, 0.0f, 1.0f, format, flags);
    if (result) {
        v[0] = uint8(round(fv[0] * 255));
        v[1] = uint8(round(fv[1] * 255));
        v[2] = uint8(round(fv[2] * 255));
        v[3] = uint8(round(fv[3] * 255));
    }
    ImGui::PopID();
    return result;
}
...
bool InputTextWithHint(ImStrv label, const char* hint, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    ImGuiEx::Label(label);
    ImGui::PushID(label);
...
bool BeginCombo(ImStrv label, ImStrv preview_value, ImGuiComboFlags flags)
{
    ...
    sprintf_s(id_buff, sizeof(id_buff), "## %.*s", label.length(), label.Begin);
...
void TextFramed(ImStrv label, const char* fmt, ...)
{
    ...
    const ImVec2 frame_size = ImGui::CalcItemSize(ImVec2(0, 0), ImGui::CalcItemWidth(), label_size.y + style.FramePadding.y * 2.0f);
    ImVec2 bb_size = frame_size;
    ...
    if (ImGui::ItemAdd(frame_bb, id, &frame_bb, ImGuiItemFlags_Inputable))
    {
        ...
        if (!text.empty()) {
            ...
            window->DrawList->AddText(g.Font, g.FontSize, draw_pos, col, text, 0.0f, &clip_rect);
        }
...
/// Derivated from imgui_widgets.cpp - ImGui::SliderScalar
